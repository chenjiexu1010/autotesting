...
public class BaiduFlowAddFriend extends SuperRunner {
        ...
        while (true) {
            try {
                ...
                if (htmlstr != null && !htmlstr.isEmpty()) {
                    ...
                    if (user == null || (user != null && (user.FriendNeedWork != "true" || user.FriendNextName == null || user.FriendNextID == null)))
                        continue;
                } else {
                    jqhelper.delay(10000);
                    continue;
                ...
                if (msg != "登录成功") {
                    ...
                    if (user.FriendNeedWork == "true" && user.FriendNextName != null && user.FriendNextID != null) {
                        ...
                        for (int j = 0; j < friendnamelist.size(); j++) {
                            ...
                            msg = AddBaiduFlowFriend(nextname);
                            if (msg == "未跳转至‘查找好友’页面") {
                                loglist.add(logInfo.setAll("申请好友", msg, nextname, "0", Vps, "", "", "", ""));
                                break;
                            } else {
                                if (msg == "该好友已添加") {
                                    submitData.SuccessNum++;
                                    submitData.FriendCurrentName = friendidlist.get(j);
                                    loglist.add(logInfo.setAll("申请好友", msg, nextname, "1", Vps, "", "", "", ""));
                                    ...
                                    submitData.SuccessNum++;
                                    submitData.FriendCurrentName = friendidlist.get(j);
                                    loglist.add(logInfo.setAll("申请好友", "", nextname, "1", Vps, "", "", "", ""));
                                } else
                                    loglist.add(logInfo.setAll("申请好友", msg, nextname, "0", Vps, "", "", "", ""));
                            }
                        }
                    ...
                    if (user.ManualReplyNextName != null && user.ManualReplyMessageContent != null) {
                        ...
                        for (int j = 0; j < manualreplynamelist.size(); j++) {
                            ...
                            String msgcontent = manualreplylist.get(j);
                            String picid = manualpiclist.get(j);
                            boolean isLoad = jqhelper.LoadPicture(picid, urlMap.get("picUrl") + picid, Device);
                ...
                submitData.setAll(user.ID, gson.toJson(loglist, logList), user.Status, user.GetTime, gson.toJson(successnamelist, listStrType), "[]", gson.toJson(manualsuccessnamelist, listStrType), gson.toJson(deletenamelist, listStrType), "[]");
    ...
    private void Initialization() throws UiObjectNotFoundException, RemoteException, IOException {
        ...
        urlMap.put("getUrl", "http://222.185.251.62:8009/api/baiduflow/list?t=30&n=1&f=30&v=" + Vps + "&d=5");
        urlMap.put("postUrl", "http://222.185.251.62:8009/api/baiduflow/updatelist/");
        jqhelper.initOCR();
    ...
    public String AddBaiduFlowFriend(String name) {
        String msg = null;
        try {
            if (isBaiduFriendAdd1()) {
            ...
            return "AddBaiduFlowFriend抛错" + e.getMessage();
        }
        return msg;
    }
